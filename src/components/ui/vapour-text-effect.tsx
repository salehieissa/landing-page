...
const renderCanvas = ({
  ...
  const canvasFont = `${framerProps.font?.fontWeight ?? 400} ${fontSize * globalDpr}px ${framerProps.font?.fontFamily ?? "sans-serif"}`;
  ...
  const { particles, textBoundaries } = createParticles(
    ctx, canvas, currentText, textX, textY, canvasFont, parsedColor,
    (framerProps.alignment as "left" | "center" | "right") || "left"
  );
  ...
  particles.forEach(particle => {
    ...
    if (shouldVaporize) {
      if (particle.speed === 0) {
        ...
        particle.shouldFadeQuickly = Math.random() > particleDensity;
      }
